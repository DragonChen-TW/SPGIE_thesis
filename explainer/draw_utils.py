...
def show_original(img, label=None, width=5):
...
def get_graph_from_image(image, desired_nodes=75, return_pos_rgb=False):
    ...
    for node in nodes:
        ...
        pos_list.append(pos_mean.tolist())
        rgb_list.append(rgb_mean.tolist())
    ...
    # centers
    centers = np.array([np.mean(np.nonzero(segments == i), axis=1)
...
#     G = nx.Graph()
#     for node in nodes:
#         print('node', nodes[node]['rgb_list'])
#         print('node', nodes[node]['pos_list'])
#         nodes[node]["rgb_list"] = np.stack(nodes[node]["rgb_list"])
#         nodes[node]["pos_list"] = np.stack(nodes[node]["pos_list"])
#         # rgb
#         rgb_mean = np.mean(nodes[node]["rgb_list"], axis=0)
...
import matplotlib.pyplot as plt
def draw_superpixel_from_graph(pos, rgb, edges, multi_graph=False, legend=None):
...
#                     c = '#{0:02x}{0:02x}{0:02x}'.format(int(feature))
                ...
                axes[idx].plot(x, y, marker='o', ms=20, color=c)
...
def load_model(fname, model, use_cuda):
    if use_cuda:
        model.load_state_dict(torch.load("{fname}.pt".format(fname=fname)))
    else:
        ckpt = torch.load("{fname}.pt".format(fname=fname), map_location=torch.device('cpu'))
